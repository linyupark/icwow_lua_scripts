...
local function levelCheck(event, player, oldLevel)
    ...
    local emailSubject = "祝贺你升级到 "..level.."!"
    local emailBody = "祝贺你升级到 "..level..", "..name.."! 这些是你的奖赏!"
    local emailTo = player:GetGUIDLow()
    local emailStationery = 61
    local emailFrom = 0
    if (level==20 or level==40 or level==60) and class~=6 then
        player:SendNotification("祝贺你升级到 "..level.."!")
...
RegisterPlayerEvent(PLAYER_EVENT_ON_LEVEL_CHANGE, levelCheck)
...
local SLATE_ENTRY = 25750

-- icwow buff start --
local BUFF_CHECK = 26393
...
-- icwow 一键升级 start --
-- 参考：https://github.com/Cloticc/Eluna-scripts/blob/master/Quality%20of%20life%20stuff/Abilities%20levelup%20v2.lua
local MaxPlayerLevel = 60    -- Max player level change to your liking.
...
-- icwow 战斗协助者（法师） start --
local FAIL_SOUND = 847
local IC_HELPER_HIRE_AURA = 62109 -- Tails Up: Aura 用一个特殊的光环来判断协助者是否已经被召唤
local IC_HELPER_ENTRY  = 669003
local IC_HELPER_WEAPONS = 22799
local IC_HELPER_SPELLS = {
...
local function initHireling(hireling, player)
    ...
    hireling:SetEquipmentSlots(IC_HELPER_WEAPONS, 0, 0)
    hireling:SetSheath(0)
end
local function dismissHireling(player)
    local aura = player:GetAura(IC_HELPER_HIRE_AURA)
    if aura then
        local hireling = aura:GetCaster()
        if hireling then
            hireling:DespawnOrUnsummon(0)
            -- player:RemoveAura(IC_HELPER_HIRE_AURA)
        -- else
...
local function spawnHirelingStats(entry, player)
    ...
    local aura = hireling:AddAura(IC_HELPER_HIRE_AURA, player)
...
local function onPlayerSpellCast(event, player, playerSpell, _)
    local spell
    local hireling = fighting and spawnHirelingStats(IC_HELPER_ENTRY, player) or nil
    ...
    if playerHpc < IC_HELPER_HPPC then
        if fighting == false then
            hireling = spawnHirelingStats(IC_HELPER_ENTRY, player)
            fighting = true
        end
        -- 治疗
    ...
    elseif fighting then
        ...
        if not hireling:IsCasting() then
            ...
            for i, name in pairs(IC_HELPER_ATK_SPELL) do
...
local function OnLogin(event, player)
    ...
    if (not player:HasItem(SLATE_ENTRY)) then
        player:AddItem(SLATE_ENTRY)
        -- player:SendBroadcastMessage("便利图纸已经发放.")
    end

    if (not player:HasSpell(28828)) then
        player:LearnSpell(28828) -- 虚空幼龙
        -- player:SendBroadcastMessage("新手冒险者坐骑技能已经传授.")
    end
...
local function onHelloMagicSlate(event, player, object)
    ...
    player:GossipMenuAddItem(4, "\"...我是速成鸡(直接60级).\"", 1, 3)
...
local function onSelectMagicSlate(event, player, object, sender, intid, code, menu_id)
    ...
    elseif intid == 4 then
        ...
        if not player:HasAura(BUFF_CHECK) then
            ...
            player:SendBroadcastMessage("你的力量还很充足")
        end
    end
end
...
RegisterPlayerEvent(PLAYER_EVENT_ON_LEARN_TALENTS, onLearnTalent)

-- icwow 战斗协助者
RegisterPlayerEvent(PLAYER_EVENT_ON_KILL_PLAYER, onPlayerDeath)
RegisterPlayerEvent(PLAYER_EVENT_ON_KILLED_BY_CREATURE, onPlayerDeath)
RegisterPlayerEvent(PLAYER_EVENT_ON_ENTER_COMBAT, onPlayerEnterCombat)
